...
void Estimator::processImage(const map<int, vector<pair<int, Vector3d>>> &image, const map<int, vector<pair<int, Vector4d>>> &lines, const std_msgs::Header &header)
{
    ...
    if(f_manager.addFeatureCheckParallax(frame_count, image, lines))      // 当视差较大时，marg 老的关键帧 // 对检测到的特征进行存放处理
...
void Estimator::processImage(const map<int, vector<pair<int, Vector3d>>> &image, const std_msgs::Header &header)
{
    ...
    if(ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            vector<pair<Vector3d, Vector3d>> corres = f_manager.getCorresponding(frame_count - 1, frame_count); // 得到两帧之间特征点关系
            Matrix3d calib_ric;
            if (initial_ex_rotation.CalibrationExRotation(corres, pre_integrations[frame_count]->delta_q, calib_ric)) // 得到camera与IMU之间的旋转偏移常量
            {
                ROS_WARN("initial extrinsic rotation calib success");
                ROS_WARN_STREAM("initial extrinsic rotation: " << endl << calib_ric);
                ric[0] = calib_ric;
    ...
    if (solver_flag == INITIAL)
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if( ESTIMATE_EXTRINSIC != 2 && (header.stamp.toSec() - initial_timestamp) > 0.1)
            {
               result = initialStructure(); // 视觉结构初始化
               initial_timestamp = header.stamp.toSec();
            }
            if(result)
            {
...
bool Estimator::initialStructure()
{
    ...
    Vector3d relative_T;
    int l;
    if (!relativePose(relative_R, relative_T, l)) // 在滑动窗口中选择两帧中有足够多特征点和视差的帧l，利用五点法恢复相对旋转和平移量
    {
    ...
    // 将相机坐标系转换到IMU坐标系中，然后再一次进行PnP求解，
    // 3D特征点还是使用之前SFM中求解出来的，后续也没有进行优化
    map<double, ImageFrame>::iterator frame_it;
...
//            if(average_parallax * 460 > 15 && m_estimator.solveRelativeRT(corres, relative_R, relative_T)) // 30
            if(average_parallax * 460 > 30 && m_estimator.solveRelativeRT(corres, relative_R, relative_T)) // 利用五点法求解相机初始位姿
            {
                l = i;
...
/////////////////////////////////////
    ...
    // printf("vio   thread -- lineFactor: %d, pointFactor:%d\n", line_m_cnt, f_m_cnt);
...
//    ROS_INFO("Points Lines Iterations : %d", static_cast<int>(summary.iterations.size()));
    ...
    // printf("vio   thread -- Points Lines solver costs: %f\n", mean_solver_time_);
...
//                        drop_set = vector<int>{0, 2};   // marg pose and feature,  !!!! do not need marg, just drop they  !!!
    ...
    // printf("vio   thread -- whole marginalization costs: %f\n", mean_marg_time_);
