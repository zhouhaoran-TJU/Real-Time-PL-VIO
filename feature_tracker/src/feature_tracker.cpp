...
void FeatureTracker::readImage(const cv::Mat &_img)
{
    ...
    cv::remap(_img, _img, undist_map1_, undist_map2_, CV_INTER_LINEAR);
    ...
    if (PUB_THIS_FRAME) {
        ...
        // cv::Mat show_img = forw_img.clone();
        // cv::cvtColor(show_img, show_img, CV_GRAY2RGB);
        // for (unsigned int j = 0; j < forw_pts.size(); j++) 
        //     cv::circle(show_img, forw_pts[j], 2, cv::Scalar(0, 255, 0), 2);
        // cv::imshow("point", show_img);
        // cv::waitKey(1);
