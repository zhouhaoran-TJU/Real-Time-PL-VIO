...
//        trackerData[i].showUndistortion("undistrotion_" + std::to_string(i));
   ...
   if (PUB_THIS_FRAME)
   {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++) {
            ...
            if (i != 1 || !STEREO_TRACK) {
                for (unsigned int j = 0; j < trackerData[i].cur_pts.size(); j++) {
                    ...
                    cv::circle(tmp_img, trackerData[i].cur_pts[j], 2, cv::Scalar(0, 0, 255), 2);
                }
                if(save_cnt%10 == 0) {
                    std::string save_name = "/home/hrzhou/1-Work/GraduateProject/Results/PL-VM/front_feature/" + std::to_string(save_cnt) + "_point.png";
    ...
    // printf("point thread ----------------- ----------------- whole point feature tracker processing costs: %f\n", mean_time);
}
...
int main(int argc, char **argv)
{
    ...
    // printf("point thread ----------------- -----------------  start point feature\n");
