...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    if (PUB_THIS_FRAME) {
        ...
        cv::Mat show_img = trackerData.curframe_->img.clone();
        ptr = cv_bridge::cvtColor(ptr, sensor_msgs::image_encodings::BGR8);
        cv::Mat tmp_img = ptr->image;
        for (int i = 0; i < NUM_OF_CAM; i++) {
            cv::cvtColor(show_img, tmp_img, CV_GRAY2RGB);
            vector<Line> l_tmp = trackerData.curframe_->vecLine;
            for(int j = 0; j<l_tmp.size(); j++) {
                Line line = l_tmp[j];
                cv::line(tmp_img, line.StartPt, line.EndPt, cv::Scalar(255, 0, 0), 2);
            }
            if(save_cnt%10 == 0) {
                std::string save_name = "/home/hrzhou/1-Work/GraduateProject/Results/PL-VM/front_feature/" + std::to_string(save_cnt) + "_line.png";
                cv::imwrite(save_name, tmp_img);
            }
            save_cnt++;
        }
        pub_image_line.publish(ptr->toImageMsg());
    }
    sum_time += t_r.toc();
    mean_time = sum_time/frame_cnt;
    // printf("line  thread ----------------- whole Line feature tracker processing costs: %f\n", mean_time);
}
...
int main(int argc, char **argv)
{
    ...
    // printf("line  thread ----------------- start line feature\n");
    ros::Subscriber sub_img = n.subscribe(IMAGE_TOPIC, 100, img_callback);

    pub_img = n.advertise<sensor_msgs::PointCloud>("linefeature", 1000);
    pub_image_line = n.advertise<sensor_msgs::Image>("line_feature_img",1000);
